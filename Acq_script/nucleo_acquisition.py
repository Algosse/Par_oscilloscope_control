...
def run(window):
    parameters = {}
    dic.load_dico('Acq_script/nucleo_acquisition.dat', parameters)
    
# =============================================================================
# Creation of the file structure
# =============================================================================
    if not(os.path.exists("Acquisition/nucleo/" + parameters['date'])): # Test if analysis path exists
        os.makedirs("Acquisition/nucleo/" + parameters['date']) # Create the folder if needed
    os.chdir("Acquisition/nucleo/" + parameters['date']) # go to the analysis folder
...
# =============================================================================
# Creation of nucleo board instance and connexion
# =============================================================================
    nucleo_board = nucleo.nucleo(**parameters) # Creation of the board Instace
    nucleo_board.connect(**parameters) # Connexion to the board
    window.ui.log.append("Wainting for reset")
    nucleo_board.read()
# =============================================================================
...
# =============================================================================
    if parameters['get trigger']:
        ...
        nucleo_board.load_key(key) # Load key
        nucleo_board.load_plaintext(plaintext[0]) # Load plaintext
        nucleo_board.start_aes() # Start AES
...
# =============================================================================
            ...
            window.oscilloscope.write("ACQ:STATE ON")
            nucleo_board.load_key(key) # Load key
            nucleo_board.load_plaintext(plaintext[i]) # Load plaintext
            nucleo_board.start_aes() # Start AES
            try:
                tmp = (window.oscilloscope.query("CURV?")).split(',') # Get trigger waveform from oscilloscope
...
# =============================================================================
    nucleo_board.disconnect() # Disconnect nucleo board
